...
public class Drive extends SubsystemBase implements Logged, AutoCloseable {
  ...
  private final PIDController turnFeedback =
      new PIDController(turnControl.kP, turnControl.kI, turnControl.kD);
  ...
  @Log.NT
  private DifferentialDriveWheelSpeeds setpointWheelSpeeds = new DifferentialDriveWheelSpeeds();
  ...
  public Drive(DriveIO hardware) {
    ...
    turnFeedback.setTolerance(
        turnControl.POSITION_TOLERANCE.in(Radians),
        turnControl.VELOCITY_TOLERANCE.in(RadiansPerSecond));

    SmartDashboard.putData("quasistatic forward", routine.quasistatic(Direction.kForward));
    SmartDashboard.putData("quasistatic backward", routine.quasistatic(Direction.kReverse));
    SmartDashboard.putData("dynamic forward", routine.dynamic(Direction.kForward));
    SmartDashboard.putData("dynamic backward", routine.dynamic(Direction.kReverse));
  }
            ...
            setChassisSpeeds(
                new ChassisSpeeds(
                    ...
                    turnFeedback.calculate(
                        ...
                        optimize(rot.get().getRadians(), pose().getRotation().getRadians())))))
  ...
  private double optimize(double setpointRadians, double measurementRadians) {
    return Math.abs(setpointRadians - measurementRadians)
            < Math.abs((setpointRadians - 2 * Math.PI) - measurementRadians)
        ? setpointRadians
